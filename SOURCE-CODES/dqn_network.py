...
class DQNAgent:
	...
	def __init__(self, env, network, buffer, epsilon=0.05, batch_size=32):
		...
		self.window = 100
		#self.reward_threshold = 195 #### THIS MEANS NOTHING TO US, WAS FOR CARTPOLE
		self.initialize()
	...
	#### used for AFTER qfunction is done training, where we assume qfunction is near optimal
	#### simply takes a step using pure-greedy action, action = argmax(Q(s,a))
	def perform_step(self):
		action = self.network.get_greedy_action(self.s_0)
		self.step_count += 1
		s_1, r, done, bulk_states = self.env.step(action)
		self.rewards += r
		self.s_0 = s_1.copy()

		if (bulk_states == {}):
			return action, s_1, self.rewards, done
		else:
			return action, bulk_states, self.rewards, done
		...
		while training:
			...
			while done == False:
				...
				if done:
					...
					#### VVV THIS MEANS NOTHING TO US, we have no "rewards threshold" 
					#### because we are not playing a game where the episode/game is over at a certain # of high reward
	...
	def initialize(self, perform=False):
		...
		self.step_count = 0
		if perform == True:
			self.s_0 = self.env.reset(perform=True)
		else:
			self.s_0 = self.env.reset()
