...
class myEnv_2weekly_vax(gym.Env):
	
	metadata = {'render.modes':['human']}
	
	def __init__(self, N, initial_state, alpha1, alpha2, alpha_linear, vaccine_startdate):
		super(myEnv_2weekly_vax, self).__init__()
		...
		self.vaccine_startdate = vaccine_startdate
		...
		self.vaccinated_perday = 310 # assume 310 people get vaccinated each day
	...
	def seir(self, rt):
		...
		if (self.max_episode_length-self.steps_remaining < self.vaccine_startdate):
			self.vaccinated_perday = 0
		else:
			self.vaccinated_perday = 310

		# get next state
		sus2 = sus - np.round(beta*inf*sus/self.N) - self.vaccinated_perday
		...
		rec2 = rec + np.round(self.gamma*inf) + self.vaccinated_perday
