...
class myEnv_varyfreq(gym.Env):
	
	metadata = {'render.modes':['human']}
	
	def __init__(self, N, initial_state, alpha1, alpha2, alpha_linear, frequency):
		super(myEnv_varyfreq, self).__init__()
	...
	def step(self, action):
		...
		# we take that action, and roll out 'frequency' days 
		bulk_reward = 0
		bulk_state = []
		for _ in np.arange(self.frequency):
